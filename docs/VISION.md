...
More concretely, Delphy Agent is becoming an **AI-native way to browse and interact with the web**. In a normal app the human drives — typing URLs, clicking pages built for human eyes — with AI bolted on as a sidebar. Delphy Agent inverts the labor while keeping the human in the seat: you express intent in plain language, the agent does the discovery and the reaching-out, and results return as **rich native views** — entity cards, profiles, players, action prompts — rather than walls of text. The web is addressed through structure agents understand (registries, MCP capabilities, manifests), not scraped HTML. The contrast that defines the product: apps that can't add an MCP server can't be open or AI-native on tools — Delphy Agent's edge is exactly that openness. Delphy MCP (the @identity registry) is the discovery+identity layer that makes this possible, which is why it ships default-on (see principle #10 and `DECISIONS.md` 2026-05-27 / 2026-06-08).
...
5. **Don't reimplement what others maintain.** When an agent CLI (`claude`, `codex`) already handles auth, native tools, sandboxing, and approvals, we drive it — we don't reinvent its loop. Driving a **subscription agent** (Codex via a ChatGPT plan, Claude Code via a Claude plan) also gives the user a flat-fee, no-API-metering path *alongside* bring-your-own-API-key — two cost models, one app (see `DECISIONS.md` 2026-06-08).
...
10. **Agent-native in the Delphy ecosystem.** Delphy Agent isn't only a client that drives AI — it's a peer in the Delphy agentic-web. It has an @identity on Delphy (the identity registry for the agentic web), exposes a machine-readable manifest plus a `skill.md` describing what it does for other agents to read, and runs an inbound MCP server so other agents can call it (open a session, send a message, query history, switch backend). Outbound MCP (principle #6) and inbound MCP are symmetric: same protocol, both directions. Beyond architectural symmetry, this principle is also the project's **distribution thesis**: Delphy Agent collapses N per-model-surface connectors (ChatGPT, Kimi, Gemini, DeepSeek…) into one BYO-model client with Delphy MCP preinstalled — see `DECISIONS.md` 2026-05-27 "Delphy Agent is dual-purpose" for the full commercial rationale.
11. **AI-native experience; plugins can contribute UI.** Capability grows by adding MCP servers — but a plugin is more than a tool list. It may also contribute **entry points** (a sidebar button, a suggested prompt — different doors to the same plugin) and **rich renderers** (a player, a media grid, a card). The plugin contract is *an MCP server + an optional UI manifest*. Plugin UI is drawn from a **constrained, trusted vocabulary** — structured descriptors our own components render, not arbitrary plugin code — so security (#1) comes before flexibility. This is the open-source flywheel: a documented contract lets anyone ship a plugin (a world-radio, a maps surface, a media concierge) that feels native. Honest boundary: some plugins *play* (a radio stream URL → `<audio>`), some only *point* (DRM-walled media like Netflix/Spotify → metadata + deep-link, not in-app playback). See `DECISIONS.md` 2026-06-08.
...
- **Not a re-skin of another agent's UI.** Driving Codex / Claude Code is about letting a user use an agent they *already pay for* (the subscription-cost pillar, principle #5), not about reframing someone else's product as ours.
