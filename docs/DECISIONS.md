...
**Lives in.** `src/core/mcp/types.ts` (transport union + optional fields); `src/core/mcp/store.ts` + `store.test.ts` (new — store layer + 15 tests); `src/core/mcp/manager.ts` + `manager.test.ts` (API additions + secret resolution + 6 new tests); `src/core/mcp/configs.ts` (deleted); `src/components/settings-modal.tsx` + `settings-modal.test.tsx` (CRUD UI + 7 new tests); `src/App.tsx` (boot path rewritten to load from store); `docs/ARCHITECTURE.md` (interim store note + slice C shipped note); `docs/SPEC.md` (mcp_servers key added to settings contract + interim implementation note); this entry. Plan: `.hermes/plans/2026-05-28_mcp-config-persistence_r1.md` (APPROVED on plan-review round 3). 208/208 vitest passing.
