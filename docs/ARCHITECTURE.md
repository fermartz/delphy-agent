...
MCP server configs are persisted in the SQLite `mcp_servers` table since BACKLOG #4 shipped (2026-05-30). The store layer (`src/core/mcp/store.ts`) handles Zod validation, default seeding, and CRUD. On first boot the layer migrates any prior `tauri-plugin-store` data (`settings.json` → `mcp_servers` key) into SQLite, flags the old file with `_mcp_migrated: true`, and treats SQLite as the sole authority thereafter. Each entry:
...
Slice B (shipped 2026-05-28) wires `mcpManager.getAllTools()` into `streamText({ tools })` via a `buildToolSet()` helper that maps each MCP tool to an AI SDK `tool({ needsApproval: true, execute: ... })`. The approval flow is a per-turn multi-streamText loop: when `finishReason === "tool-calls"` with pending approvals, the session awaits user verdicts, appends `ToolApprovalResponse[]` to the messages array, and calls `streamText` again. `mcpManager.callTool(namespacedName, args)` executes approved tool calls with a 30-second timeout. `APPROVAL_CYCLE_CAP = 5` limits chained tool calls per user turn. See `docs/DECISIONS.md` 2026-05-28 entry for the full design + alternatives considered. Slice C (shipped 2026-05-28) replaced the hardcoded `configs.ts` with user-managed config persistence (originally backed by `tauri-plugin-store`; **migrated to SQLite in BACKLOG #4 on 2026-05-30** — see the "Persistence" section above) + a full CRUD UI in Settings (add/edit/remove/restart/toggle) + `${secret:key}` resolution against the OS keychain at boot time + inline API key rejection at save time. Remote transports (Streamable HTTP + legacy SSE) boot alongside stdio since BACKLOG #9.A (2026-06-08); OAuth-authed remote servers are still pending (#9.B).

**Per-tool enable/disable + tool-payload stability (BACKLOG #18/#20, 2026-07-06).** Users can disable individual tools of a connected server from Settings → Plugins (`McpServerConfig.disabledTools`). Filtering happens once at the manager boundary — `getAllTools()` returns the model-facing view (disabled removed, sorted by serverId then tool name for a byte-stable payload), `getServerTools(id)` returns the unfiltered list for the UI, and `callTool()` rejects disabled tools defensively. A toggle is a read-time filter (`setDisabledTools`): no server restart, just a bump of the manager's revision counter. The adapter memoizes the built `ToolSet` + system-prompt tool context + a tools-token estimate on that revision, so the tools block sent per turn is byte-identical across normal turns (a prompt-cache prefix) and rebuilds only on user MCP changes. The tools-payload estimate feeds a shared context-size helper behind both the StatusBar `context_usage` event and `/status`. See `docs/DECISIONS.md` 2026-07-06.
...
-- Persistent memory (astra pattern: one row per scope, whole-content replace,
-- 3000-char cap). v1 only writes the "global" row; per-session scope reserved.
memory(
  id TEXT PRIMARY KEY,              -- "global" or session_id
