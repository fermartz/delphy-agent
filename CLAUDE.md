...
**Why this product exists (strategic framing).** Delphy Agent serves a dual purpose: it is a genuinely useful standalone multi-model agent hub *and* the universal distribution channel for the maintainer's Delphy @identity registry (delphy.network). Without it, reaching the long tail of model surfaces (ChatGPT, Kimi, Gemini, DeepSeek) requires building and maintaining N custom connectors forever; with it, one install gives any user any model + Delphy MCP default-on + every other MCP server they want. This duality is non-negotiable — the standalone product must be useful even to users who don't care about Delphy, and Delphy MCP is a *committed* default-on (not an opt-in plugin). **Implementation status:** not yet shipped — today only `Fetch` is seeded as a default server; making Delphy MCP default-on is **BACKLOG #24** (Phase 0 of the AI-native-web journey). See `docs/DECISIONS.md` 2026-05-27 "Delphy Agent is dual-purpose" for the full rationale and constraints this locks in.

**Current state (high-level — `memory/MEMORY.md` is the authoritative living snapshot; keep it current and treat it as source of truth over this paragraph).** Tauri v2 + React 19 + TS + Tailwind v4 + shadcn + Biome + Vitest. A **multi-provider** direct-API hub via Vercel AI SDK v6 + per-provider `ProviderProfile` — 8 providers shipped (natives `anthropic`/`openai`/`google`/`xai` + first-class OpenAI-compatible `openrouter`/`kimi`/`deepseek`/`groq` + a generic Custom OpenAI-compatible profile). Real token streaming, three-tier system prompt, auto-compaction + `/compact` + `/status`. Secret store via OS keychain. **SQLite** session persistence + resume + built-in `update_memory` tool. **MCP fully wired** — Rust stdio bridge + tool/approval flow + CRUD UI + fail-fast on child exit; **remote transports** (Streamable HTTP + legacy SSE) via the MCP SDK over a `tauri-plugin-http` proxied fetch (BACKLOG #9.A, 2026-06-08, verified against CoinGecko's public Streamable HTTP MCP; OAuth pending #9.B). Theme system (6 built-ins). Tabbed Settings (Providers/Models/Plugins/Appearance). Echo adapter = boot fallback + test fixture. The chat UI is decomposed: `App.tsx` is a ~450-line shell composing focused hooks (`src/hooks/use-{session,mcp-servers,providers,themes,chat-scroll}.ts`) + presentational components (`src/components/{app-header,chat-stream,chat-message,composer,boot-banner,toast}.tsx`) + a pure event reducer (`src/core/chat/items-reducer.ts`); the five non-chat surfaces are `React.memo`-wrapped so streaming re-renders only the chat. A second backend has landed: **Codex (agent-cli) — Slice A** drives `codex mcp-server` over the existing MCP bridge (read-only turn loop, selectable via a Settings Backend toggle + working directory). **Webview security hardening (2026-07-02):** all provider egress is proxied through the Tauri HTTP boundary (no direct webview key egress), a **strict CSP + devCsp** closes the XSS→RCE path, a shared **SSRF guard** (`src/core/net/validate-egress-url.ts`) gates user- and content-supplied URLs, remote markdown images are **load-on-consent** (per-host trust), and **CI** (GitHub Actions) gates lint/typecheck/test/build + cargo. Latest work: **MCP tool-context token efficiency** (BACKLOG #18+#20, audit #9, 2026-07-06 — build-review APPROVED): per-tool enable/disable from the Plugins tab (`disabledTools` in the config JSON; read-time filter at the manager boundary, no server restart), cache-stable tool ordering + a revision-memoized tool set (byte-stable tools payload = prompt-cache prefix), and the tools payload counted in a shared context estimate behind both the StatusBar and `/status` (see `docs/DECISIONS.md` 2026-07-06). Prior: a **public-repo security-audit remediation** (2026-07-02, one session — see `.hermes/reviews/2026-07-02_public-repo-audit.md`) cleared 8 findings, each Codex-reviewed + pushed: CI (`3f3db8a`); the egress-hardening slice (`c876375`/`abbc0fe`/`0806c99`/`aa4a99f`); atomic `replaceMessages` (`6f1f728`, first Rust tests + `sqlx`); doc drift (`aeaf083`); Rust tests for secrets/mcp_bridge (`ff8b776`); compaction re-entrancy guard (`aa4c9ff`). Remaining audit items are slice-sized (SSRF resolve-and-pin, MCP lifecycle tests, boot self-recovery). Prior: BACKLOG #7 Codex adapter Slice A (`07af8c6`…`7ae8b76`, 2026-06-08). See `memory/delphy-agent-tasks.md` for the live backlog and `docs/DECISIONS.md` for the decision log.
...
- Codex agent-cli backend: **Slice A SHIPPED 2026-06-08** (read-only turn loop over `codex mcp-server`). Slice B (approvals + `workspace-write` sandbox) + Slice C (feed Codex the user's MCP servers) **not yet** — BACKLOG #7. Claude Code backend still deferred (`claude mcp serve` exposes tools, not a driveable agent — see `docs/DECISIONS.md` 2026-05-26).
...
- ~~No per-tool enable/disable~~ — **SHIPPED 2026-07-06 (BACKLOG #18+#20):** per-tool toggles in Settings → Plugins, read-time filter (no restart), revision-memoized byte-stable tools payload, tools counted in the context estimate. Progressive tool disclosure (tool-search indirection) still **not** — **BACKLOG #19**, deferred past the catalog (#11); wrong tradeoff at ~15-tool scale.
